...
describe('SLH-DSA (SPHINCS+)', () => {
    describe('sphincs256', () => {
        describe('fast variant', () => {
            ...
            const { secretKey, publicKey } = sphincs256.fast.generateKeyPair(TEST_VECTOR.sphincs256Seed);
            expect(secretKey).toBeInstanceOf(Uint8Array);
            expect(publicKey).toBeInstanceOf(Uint8Array);
            
            const signature = sphincs256.fast.sign(TEST_VECTOR.message, secretKey);
        ...
        it('should validate signing input', () => {
            const { secretKey } = sphincs256.fast.generateKeyPair();
            expect(() => sphincs256.fast.sign('invalid', secretKey))
        ...
        it('should generate correct size outputs', () => {
            const { secretKey, publicKey } = sphincs256.fast.generateKeyPair();
            expect(publicKey.length).toBe(TEST_VECTOR.sphincs256.fast.publicKeySize);
            expect(secretKey.length).toBe(TEST_VECTOR.sphincs256.fast.secretKeySize);
    ...
    describe('small variant', () => {
        it('should sign and verify', () => {
            const { secretKey, publicKey } = sphincs256.small.generateKeyPair(TEST_VECTOR.sphincs256Seed);
            expect(secretKey).toBeInstanceOf(Uint8Array);
            expect(publicKey).toBeInstanceOf(Uint8Array);
            
            const signature = sphincs256.small.sign(TEST_VECTOR.message, secretKey);
            expect(signature).toBeInstanceOf(Uint8Array);
        ...
        it('should validate signing input', () => {
            const { secretKey } = sphincs256.small.generateKeyPair();
            expect(() => sphincs256.small.sign('invalid', secretKey))
                .toThrow('message must be a Uint8Array');
        ...
        // Test key and signature sizes
        it('should generate correct size outputs', () => {
            const { secretKey, publicKey } = sphincs256.small.generateKeyPair();
            expect(publicKey.length).toBe(TEST_VECTOR.sphincs256.small.publicKeySize);
            expect(secretKey.length).toBe(TEST_VECTOR.sphincs256.small.secretKeySize);
